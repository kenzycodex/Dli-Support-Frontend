// components/common/search-with-suggestions.tsx (FIXED - No unnecessary refetches)
...
export function SearchWithSuggestions({
  ...
  // Don't debounce for suggestions display - only for actual search execution
  const debouncedSearchValue = useDebounce(value, 500) // Longer debounce for actual search

  // Build combined suggestions list - no debouncing here for instant UI feedback
  ...
  // Only trigger search when debounced value changes and has content
  useEffect(() => {
    if (debouncedSearchValue.trim() && debouncedSearchValue !== value) {
      // Only search if the debounced value is different from current value
      // This prevents search on every keystroke
      onSearch(debouncedSearchValue.trim())
    }
  }, [debouncedSearchValue]) // Remove onSearch from dependencies to prevent loops
  ...
  // Handle suggestion click (immediate)
  const handleSuggestionClick = useCallback((suggestion: SearchSuggestion) => {
    onChange(suggestion.text)
    onSearch(suggestion.text) // Immediate search, no debounce
    setIsOpen(false)
    setHighlightedIndex(-1)
  }, [onChange, onSearch])

  // Handle input change (only update value, don't trigger search)

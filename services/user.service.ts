// services/user.service.ts - FIXED with proper User type export

import { apiClient, type StandardizedApiResponse } from '@/lib/api'

// FIXED: Export User interface from this file
...
class UserService {
  ...
  ): Promise<StandardizedApiResponse<UserListResponse>> {
    try {
  ...
  async createUser(
    ...
    StandardizedApiResponse<{ user: User; email_sent?: boolean; temporary_password?: string }>
  > {
    try {
  ...
  ): Promise<StandardizedApiResponse<{ user: User }>> {
    try {
  ...
  > {
    try {
      ...
      const response = await this.apiClient.post<{
        ...
        email_sent?: boolean;
        temporary_password?: string;
      }>(`/admin/users/${id}/reset-password`, requestData);
  ...
  ): Promise<StandardizedApiResponse<{ affected_count: number; action: string }>> {
    try {
      console.log(
        '🔄 UserService: Performing bulk action:',
        data.action,
        'on',
      ...
      const response = await this.apiClient.post<{ affected_count: number; action: string }>(
  ...
  async bulkCreate(data: BulkCreateRequest): Promise<StandardizedApiResponse<BulkCreateResponse>> {
    try {
      ...
      if (data.generate_passwords !== undefined) {
        formData.append('generate_passwords', data.generate_passwords.toString());
      }

      const response = await this.apiClient.post<BulkCreateResponse>(
  ...
  async getUserStats(
    options: RequestOptions = {}
  ): Promise<StandardizedApiResponse<{ stats: UserStats }>> {
    try {
      const { forceRefresh = false } = options;
  ...
  async exportUsers(
    ...
    StandardizedApiResponse<{ users: User[]; exported_at: string; total_exported: number }>
  > {
    try {

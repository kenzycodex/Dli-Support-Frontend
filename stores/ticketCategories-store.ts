// stores/ticketCategories-store.ts - FIXED: Stable selectors to prevent infinite loops
...
// FIXED: Individual selectors with proper memoization (like ticket store pattern)
export const useCategoryById = (id: number) => {
	return useTicketCategoriesStore(
		useCallback((state) => {
			try {
				// First check cache
...
export const useCategoriesLoading = (type: keyof LoadingState = 'list') => {
	return useTicketCategoriesStore(useCallback((state) => state.loading[type], [type]))
}

export const useCategoriesError = (type: keyof ErrorState = 'list') => {
	return useTicketCategoriesStore(useCallback((state) => state.errors[type], [type]))
}

// FIXED: Data selectors with stable references (exactly like ticket store pattern)
export const useCategoriesData = () => {
	// FIXED: Use individual selectors instead of object selector to prevent recreation
	const categories = useTicketCategoriesStore(useCallback((state) => state.categories, []))
	const currentCategory = useTicketCategoriesStore(useCallback((state) => state.currentCategory, []))
	const overview = useTicketCategoriesStore(useCallback((state) => state.overview, []))
	const filters = useTicketCategoriesStore(useCallback((state) => state.filters, []))
	const loading = useTicketCategoriesStore(useCallback((state) => state.loading, []))
	const errors = useTicketCategoriesStore(useCallback((state) => state.errors, []))
	const lastFetch = useTicketCategoriesStore(useCallback((state) => state.lastFetch, []))
	...
	return useMemo(() => ({
		...
		errors,
		lastFetch
	}), [categories, currentCategory, overview, filters, loading, errors, lastFetch])
}

export const useCategoriesSelectors = () => {
	const categories = useTicketCategoriesStore(useCallback((state) => state.categories, []))
	const selectedCategories = useTicketCategoriesStore(useCallback((state) => state.selectedCategories, []))
	...
	return useMemo(() => ({
		...
		selectedCategoriesArray: Array.from(selectedCategories)
...
export const useCategoriesActions = () => {
	return useTicketCategoriesStore(useCallback((state) => state.actions, []))
}

export const useCategoriesCache = () => {
	const categoryCache = useTicketCategoriesStore(useCallback((state) => state.categoryCache, []))
	
	return useMemo(() => ({
		categoryCache,
		getCacheStats: () => ({
			totalCachedCategories: categoryCache.size,
			oldestCacheEntry: categoryCache.size > 0 
				? Math.min(...Array.from(categoryCache.values()).map(c => c.lastUpdate))
				: Date.now(),
		}),
	}), [categoryCache])
}
